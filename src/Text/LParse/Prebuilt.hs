...
peek :: (TokenStream s) => (t -> Bool) -> ParserError e -> Parser r e (s t) ()
peek c = cParse (c . top) noop

-- | A parser that parses nothing, but only succeeds if the input is empty
eoi :: Parser r e [t] ()
eoi = cParse null noop UnconsumedTokens

-- *** Consumers

-- | A parser that consumes the whole input and discards it, successfully
discard :: Parser r e [t] ()
discard = void full

-- | Succeeds exactly if the input begins with the given sequence. On success, consumes that sequence
consume :: (Eq t, Show (s t), TokenStream s) => s t -> Parser r e (s t) ()
consume pre = cParse ((&&) <$> (and . T.zipWith (==) pre) <*> ((>= length pre) . length)) (pParse (T.drop (length pre)) noop) (UnexpectedToken $ show pre)

-- | Consumes exactly the given input and then returns the given constant result
consumeReturn :: (Eq t, Show (s t), TokenStream s) => s t -> a -> Parser r e (s t) a
...
consumeSingle :: (Eq t, Show t, TokenStream s) => t -> Parser r e (s t) ()
consumeSingle t = cParse (\s -> not (null s) && top s == t) (pParse rest noop) (UnexpectedToken $ show t)

-- | Consumes exactly the given token and then returns the given constant result
consumeSReturn :: (Eq t, Show t, TokenStream s) => t -> a -> Parser r e (s t) a
...
tokenReturn :: (TokenStream s) => Parser r e (s a) a
...
tokenParse :: (TokenStream s) => (t -> a) -> Parser r e (s t) a
tokenParse f = Parser (\s -> DCont (\btr etr -> if null s then etr UnexpectedEOI else btr (f $ top s,rest s)))

-- | A parser that always succeeds with the given function
success :: (t -> (a,t)) -> Parser r e t a
success = Parser . (return .)

-- ** Entity Parsers

-- | Extracts the first digit and returns it
digit :: Parser r e String Integer
digit = read . return <$> cParse (\s -> not (null s) && isDigit (head s)) tokenReturn (UnexpectedToken "digit")

-- | Extracts the first digit and returns it
letter :: Parser r e String Char
letter = cParse (\s -> not (null s) && isLetter (head s)) tokenReturn (UnexpectedToken "letter")

-- | Extracts the first word (i.e. contiguous string of letters) from the input and returns it
word :: Parser r e String String
word = some letter 

-- | Extracts the first integer (i.e. contiguous string of digits) from the input and returns it
integer :: Parser r e String Integer
...
sInteger :: Parser r e String Integer
...
bDigit :: Integer -> Parser r e Integer Integer
bDigit b = cParse (> 0) (success (\i -> (i `mod` b,i `div` b))) (UnexpectedToken "digit")

-- | Parses an integer by removing a single digit in the given base from it. Zero is considered to have no digits
bDigits :: Integer -> Parser r e Integer [Integer]
...
cParse :: (t -> Bool) -> Parser r e t a -> ParserError e -> Parser r e t a
...
sepSome :: Parser r e t () -> Parser r e t a -> Parser r e t [a]
sepSome sep p = ((:) <$> p <*> many (sep >> p)) <|> fmap return p

-- | Same as @sepSome@, but allows empty lists
sepMany :: Parser r e t () -> Parser r e t a -> Parser r e t [a]
